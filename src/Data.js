export const categories = [
    {
      id: 1,
      name: "Tất cả",
      products: [
        { id: 1, name: "Trà đào cam xả", price: "50,000₫", image: "data:image/jpeg;base64," },
        { id: 2, name: "Trà xanh kem cheese", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 3, name: "Trà sữa matcha", price: "55,000₫", image: "/assets/Menu.png" },
        { id: 4, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 5, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 6, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 7, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 8, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 9, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 10, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 11, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 12, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 13, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 14, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 16, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" },
        { id: 15, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" }

      ]
    },
    {
      id: 2,
      name: "Trà sữa",
      products: [
        { id: 3, name: "Trà sữa matcha", price: "55,000₫", image: "/assets/Menu.png" },
        { id: 4, name: "Trà sữa trân châu", price: "50,000₫", image: "/assets/Menu.png" }
      ]
    },
    {
      id: 3,
      name: "Nước ép",
      products: [
        { id: 5, name: "Nước ép cam", price: "45,000₫", image: "/assets/Menu.png" },
        { id: 6, name: "Nước ép táo", price: "50,000₫", image: "/assets/Menu.png" }
      ]
    }
  ];

  export const tables = [
    { id: 1, name: "Bàn 1", status: "empty" },
    { id: 2, name: "Bàn 2", status: "occupied" },
    { id: 3, name: "Bàn 3", status: "empty" },
    { id: 4, name: "Bàn 4", status: "occupied" },
    { id: 5, name: "Bàn 5", status: "empty" },
    { id: 6, name: "Bàn 6", status: "occupied" },
    { id: 7, name: "Bàn 7", status: "empty" },
    { id: 8, name: "Bàn 8", status: "occupied" },
    { id: 9, name: "Bàn 9", status: "empty" },
    { id: 10, name: "Bàn 10", status: "occupied" },
    { id: 11, name: "Bàn 11", status: "empty" },
    { id: 12, name: "Bàn 12", status: "occupied" },
    { id: 13, name: "Bàn 13", status: "empty" },
    { id: 14, name: "Bàn 14", status: "occupied" },
    { id: 15, name: "Bàn 15", status: "empty" },
    { id: 16, name: "Bàn 16", status: "occupied" },
    { id: 17, name: "Bàn 17", status: "empty" },
    { id: 18, name: "Bàn 18", status: "occupied" },
    { id: 19, name: "Bàn 1", status: "empty" },
    { id: 20, name: "Bàn 2", status: "occupied" },
    { id: 21, name: "Bàn 3", status: "empty" },
    { id: 22, name: "Bàn 4", status: "occupied" },
    { id: 23, name: "Bàn 5", status: "empty" },
    { id: 24, name: "Bàn 6", status: "occupied" },
    { id: 25, name: "Bàn 7", status: "empty" },
    { id: 26, name: "Bàn 8", status: "occupied" },
    { id: 27, name: "Bàn 9", status: "empty" },
    { id: 28, name: "Bàn 10", status: "occupied" },
    { id: 29, name: "Bàn 11", status: "empty" },
    { id: 30, name: "Bàn 12", status: "occupied" },
    { id: 31, name: "Bàn 13", status: "empty" },
    { id: 32, name: "Bàn 14", status: "occupied" },
    { id: 33, name: "Bàn 15", status: "empty" },
    { id: 34, name: "Bàn 16", status: "occupied" },
    { id: 35, name: "Bàn 17", status: "empty" },
    { id: 36, name: "Bàn 18", status: "occupied" },
    { id: 37, name: "Bàn 2", status: "occupied" },
    { id: 38, name: "Bàn 3", status: "empty" },
    { id: 39, name: "Bàn 4", status: "occupied" },
    { id: 40, name: "Bàn 5", status: "empty" },
    { id: 41, name: "Bàn 6", status: "occupied" },
    { id: 42, name: "Bàn 7", status: "empty" },
    { id: 43, name: "Bàn 8", status: "occupied" },
    { id: 44, name: "Bàn 9", status: "empty" },
    { id: 45, name: "Bàn 10", status: "occupied" },
    { id: 46, name: "Bàn 11", status: "empty" },
    { id: 47, name: "Bàn 12", status: "occupied" },
    { id: 48, name: "Bàn 13", status: "empty" },
    { id: 49, name: "Bàn 14", status: "occupied" },
    { id: 50, name: "Bàn 15", status: "empty" },
    { id: 51, name: "Bàn 16", status: "occupied" },
    { id: 52, name: "Bàn 17", status: "empty" },
    { id: 53, name: "Bàn 18", status: "occupied" },
    { id: 54, name: "Bàn 1", status: "empty" },
  ];
  export const productsData = [
    { id: 1, name: "Trà đào cam xả", quantity: 1, price: 50000 },
    { id: 2, name: "Trà xanh kem cheese", quantity: 1, price: 50000 },
    { id: 3, name: "Trà sữa trân châu", quantity: 1, price: 50000 },
    { id: 4, name: "Trà đào bốn mùa", quantity: 1, price: 50000 },
    { id: 5, name: "Nước ép dứa", quantity: 1, price: 50000 },
    { id: 6, name: "Trà đào bốn mùa", quantity: 1, price: 50000 },
    { id: 7, name: "Nước ép dứa", quantity: 1, price: 50000 },
    { id: 8, name: "Trà đào bốn mùa", quantity: 1, price: 50000 },
    { id: 9, name: "Nước ép dứa", quantity: 1, price: 50000 },
  ];

// Dữ liệu danh sách bàn
// export const tableData = [
//   { id: 1, name: "Bàn 1", items: 5 },
//   { id: 2, name: "Bàn 2", items: 3 },
//   { id: 3, name: "Bàn 1", items: 5 },
//   { id: 4, name: "Bàn 2", items: 3 },
//   { id: 5, name: "Bàn 1", items: 5 },
//   { id: 6, name: "Bàn 2", items: 3 },
//   { id: 7, name: "Bàn 1", items: 5 },
//   { id: 8, name: "Bàn 2", items: 3 },
//   { id: 9, name: "Bàn 1", items: 5 },
//   { id: 10, name: "Bàn 2", items: 3 },
//   { id: 11, name: "Bàn 1", items: 5 },
//   { id: 12, name: "Bàn 2", items: 3 },
//   { id: 13, name: "Bàn 1", items: 5 },
//   { id: 14, name: "Bàn 2", items: 3 },
//   { id: 15, name: "Bàn 1", items: 5 },
//   { id: 16, name: "Bàn 2", items: 3 },
// ];
export const tableData = [
  {
    id: 1,
    name: "Bàn 1",
    customerName: "Nguyen Van A",
    phone: "0388888888",
    bartender: "Nguyen Van B",
    items: [
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
    ],
  },
  {
    id: 2,
    name: "Bàn 2",
    customerName: "Tran Thi C",
    phone: "0377777777",
    bartender: "Tran Van D",
    items: [
      { name: "Trà sữa trân châu", quantity: 2, note: "Không đá" },
      { name: "Cà phê đen", quantity: 1, note: "" },
    ],
  },
  {
    id: 3,
    name: "Bàn 1",
    customerName: "Nguyen Van A",
    phone: "0388888888",
    bartender: "Nguyen Van B",
    items: [
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
    ],
  },
  {
    id: 4,
    name: "Bàn 2",
    customerName: "Tran Thi C",
    phone: "0377777777",
    bartender: "Tran Van D",
    items: [
      { name: "Trà sữa trân châu", quantity: 2, note: "Không đá" },
      { name: "Cà phê đen", quantity: 1, note: "" },
    ],
  },
  {
    id: 5,
    name: "Bàn 1",
    customerName: "Nguyen Van A",
    phone: "0388888888",
    bartender: "Nguyen Van B",
    items: [
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
    ],
  },
  {
    id: 6,
    name: "Bàn 2",
    customerName: "Tran Thi C",
    phone: "0377777777",
    bartender: "Tran Van D",
    items: [
      { name: "Trà sữa trân châu", quantity: 2, note: "Không đá" },
      { name: "Cà phê đen", quantity: 1, note: "" },
    ],
  },
  {
    id: 7,
    name: "Bàn 1",
    customerName: "Nguyen Van A",
    phone: "0388888888",
    bartender: "Nguyen Van B",
    items: [
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
    ],
  },
  {
    id: 8,
    name: "Bàn 2",
    customerName: "Tran Thi C",
    phone: "0377777777",
    bartender: "Tran Van D",
    items: [
      { name: "Trà sữa trân châu", quantity: 2, note: "Không đá" },
      { name: "Cà phê đen", quantity: 1, note: "" },
    ],
  },
  {
    id: 9,
    name: "Bàn 1",
    customerName: "Nguyen Van A",
    phone: "0388888888",
    bartender: "Nguyen Van B",
    items: [
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
    ],
  },
  {
    id: 10,
    name: "Bàn 2",
    customerName: "Tran Thi C",
    phone: "0377777777",
    bartender: "Tran Van D",
    items: [
      { name: "Trà sữa trân châu", quantity: 2, note: "Không đá" },
      { name: "Cà phê đen", quantity: 1, note: "" },
    ],
  },
  {
    id: 11,
    name: "Bàn 1",
    customerName: "Nguyen Van A",
    phone: "0388888888",
    bartender: "Nguyen Van B",
    items: [
      { name: "Trà đào cam xả", quantity: 3, note: "Ít đá" },
      { name: "Bạc xỉu", quantity: 2, note: "" },
      { name: "Nước ép dứa", quantity: 1, note: "" },
    ],
  },
  {
    id: 12,
    name: "Bàn 2",
    customerName: "Tran Thi C",
    phone: "0377777777",
    bartender: "Tran Van D",
    items: [
      { name: "Trà sữa trân châu", quantity: 2, note: "Không đá" },
      { name: "Cà phê đen", quantity: 1, note: "" },
    ],
  },

];

// data.js
export const products = [
  { id: 1, name: "Sản phẩm 1", price: "10.000", category: "Nước ép" },
  { id: 2, name: "Sản phẩm 2", price: "12.000", category: "Trà sữa" },
  { id: 3, name: "Sản phẩm 3", price: "15.000", category: "Cà phê" },
  { id: 4, name: "Sản phẩm 4", price: "18.000", category: "Nước ép" },
  { id: 5, name: "Sản phẩm 5", price: "20.000", category: "Sinh tố" },
  { id: 6, name: "Sản phẩm 6", price: "25.000đ", category: "Nước ép" },
  { id: 7, name: "Sản phẩm 7", price: "30.000", category: "Trà sữa" },
  { id: 8, name: "Sản phẩm 8", price: "22.000 ", category: "Cà phê" },
  { id: 9, name: "Sản phẩm 9", price: "35.000đ", category: "Sinh tố" },
  { id: 10, name: "Sản phẩm 10", price: "40.000đ", category: "Nước ép" },
  { id: 11, name: "Sản phẩm 11", price: "45.000đ", category: "Cà phê" },
  { id: 12, name: "Sản phẩm 12", price: "50.000đ", category: "Trà sữa" },
  { id: 13, name: "Sản phẩm 13", price: "55.000đ", category: "Sinh tố" },
  { id: 14, name: "Sản phẩm 14", price: "60.000đ", category: "Nước ép" },
];

export const category = [
  { id: 1, category: "Nước ép" },
  { id: 2, category: "Trà sữa" },
  { id: 3, category: "Cà phê" },
  { id: 4, category: "Sinh tố" },
];


